...
export default {
  ...
  methods: {
    ...
    // 获取页面广告内容
    get_contents() {
      this.$axios.get("contents/").then(res => {
        const data = res.data.data || [];
    
        // 判空保护
        if (!Array.isArray(data) || data.length === 0) {
          console.warn("⚠️ 后端未返回任何内容数据");
          return;
        }
    
        this.contents = data;
    
        // 使用可选链 ?. 防止 undefined 报错
        this.index_lbt = data[0] || {};
        this.index_kx = data[1] || {};
        this.index_ytgg = data[2]?.contents?.[0] || {};
        this.index_1f_logo = data[3]?.contents?.[0] || {};
        this.index_1f_ssxp = data[6]?.contents || [];
        this.index_1f_cxdj = data[7]?.contents || [];
        this.index_1f_sjpj = data[8]?.contents || [];
    
        this.index_2f_logo = data[9]?.contents?.[0] || {};
        this.index_2f_jjhg = data[12]?.contents || [];
        this.index_2f_cxdj = data[13]?.contents || [];
    
        this.index_3f_logo = data[14]?.contents?.[0] || {};
        this.index_3f_shyp = data[17]?.contents || [];
        this.index_3f_cfyp = data[18]?.contents || [];
      }).catch(err => {
        console.error("❌ 获取首页内容失败：", err);
      });
    }
  },
  components: {
